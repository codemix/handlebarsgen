// Generated by CoffeeScript 1.6.1
(function() {
  var CLI, Generator, Targets, fs, mkdirp, optimist, path;

  fs = require("fs");

  path = require("path");

  optimist = require("optimist");

  mkdirp = require("mkdirp");

  Generator = require("./generator");

  Targets = require("./targets");

  /*
  Command line interface for handlebars-gen
  */


  module.exports = CLI = (function() {
    /*
    Initialize the CLI based on the command line options
    */

    function CLI(options) {
      if (options == null) {
        options = optimist.argv;
      }
      if (options.target != null) {
        this.target = Targets[options.target];
        if (this.target == null) {
          throw new Error("No such target: " + options.target);
        }
      } else {
        this.target = Targets.php;
      }
      this.help = options.help || false;
      this.version = options.version || false;
      if (options.bare != null) {
        this.bare = options.bare;
      } else {
        this.bare = false;
      }
      if (options.pattern != null) {
        this.pattern = new RegExp(options.pattern);
      } else {
        this.pattern = this.target.pattern;
      }
      this.outputMeta = options.meta || false;
      this.extname = options.extname || this.target.extname;
      if (options.output != null) {
        this.output = path.resolve(options.output);
      } else {
        this.output = null;
      }
      if (options._.length) {
        this.input = options._;
      } else {
        this.input = null;
      }
      this.generator = new Generator({
        target: this.target
      });
    }

    /*
    Display the help output
    */


    CLI.prototype.showHelp = function() {
      var pack;
      pack = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'package.json'), 'utf8'));
      return process.stderr.write("" + pack.name + " v" + pack.version + " by " + pack.author.name + "\n\n" + pack.description + "\n\nUsage: handlebars-gen --target php --output directory/path INFILE...\n\nOptions:\n\n  INFILE              The file(s) or folder(s) to process\n\n  --output DIR        The directory or file to output the contents to.\n                      If none is given, output will be written to stdout.\n\n  --target NAME       The name of the transpile target to use, e.g. 'php' or 'php-yii'\n\n  --pattern PATTERN   The regular expression that should be used when looking for files.\n\n  --extname EXTNAME   The file extension to use when generating output, defaults to .html\n\n  --bare              If set, template output will not be decorated with headers and footers.\n\n  --meta FILENAME     Dump the meta data to the given filename\n\n  --version           Print the version number\n\n  --help              Display this help screen\n");
    };

    /*
    Display the version number
    */


    CLI.prototype.showVersion = function() {
      var pack;
      pack = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'package.json'), 'utf8'));
      return process.stdout.write("" + pack.version + "\n");
    };

    /*
    Run the command
    */


    CLI.prototype.run = function() {
      var files, item, resolved, _i, _len, _ref;
      if (this.help) {
        return this.showHelp();
      }
      if (this.version) {
        return this.showVersion();
      }
      files = [];
      if (this.input === null) {
        files.push = {
          id: "__stdin__",
          name: "__stdin__",
          folder: "__stdin__",
          original: "__stdin__",
          extname: "",
          ast: generator.generate(fs.readFileSync('/dev/stdin', 'utf8'))
        };
      } else {
        _ref = this.input;
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          item = _ref[_i];
          resolved = path.resolve(item);
          if (fs.statSync(item).isDirectory()) {
            files.push.apply(files, this.generateDir(resolved));
          } else {
            files.push(this.generateFile(path.dirname(resolved), path.dirname(resolved), path.basename(resolved)));
          }
        }
      }
      return this.writeOutput(files);
    };

    /*
    Write the output
    */


    CLI.prototype.writeOutput = function(files) {
      var data, exists, file, isDirectory, isFile, stats, targetFile, targetFolder, wrapperTemplates, _i, _len;
      if (this.bare) {
        wrapperTemplates = {};
      } else {
        wrapperTemplates = this.generator.compileWrapperTemplates();
      }
      if (this.output != null) {
        exists = fs.existsSync(this.output);
        if (exists) {
          stats = fs.statSync(this.output);
          isDirectory = stats.isDirectory();
          isFile = stats.isFile();
        } else {
          isFile = /\.(\w+)$/.test(this.output);
          isDirectory = !isFile;
        }
      }
      if (isDirectory) {
        for (_i = 0, _len = files.length; _i < _len; _i++) {
          file = files[_i];
          targetFolder = path.join(this.output, file.folder);
          mkdirp.sync(targetFolder);
          targetFile = path.join(targetFolder, "" + file.name + this.extname);
          fs.writeFileSync(targetFile, this.generator.wrapFile(file, wrapperTemplates));
        }
        if (this.outputMeta) {
          this.writeMeta(files);
        }
        return;
      }
      if (this.output != null) {
        if (files.length > 1) {
          data = this.target.combine(files, wrapperTemplates);
        } else {
          data = this.generator.wrapFile(file, wrapperTemplates);
        }
        targetFolder = mkdirp.sync(path.dirname(this.output));
        fs.writeFileSync(this.output, data);
      } else {
        data = this.target.combine(files, wrapperTemplates);
        process.stdout.write("" + data + "\n");
      }
      if (this.outputMeta) {
        if (this.outputMeta) {
          return this.writeMeta(files);
        }
      }
    };

    /*
    Write the meta data to a file
    */


    CLI.prototype.writeMeta = function(files) {
      return fs.writeFileSync(this.outputMeta, JSON.stringify(files, null, 2));
    };

    /*
    Generate a file
    */


    CLI.prototype.generateFile = function(wd, folder, filename) {
      var ast, extname, name, original;
      extname = path.extname(filename);
      name = path.basename(filename, extname);
      original = path.join(wd, folder, filename);
      try {
        ast = this.generator.generate(fs.readFileSync(original, "utf8"));
      } catch (error) {
        process.stdin.write(["Parse Error in " + original + ": expected", error.expected.join(", "), "found", error.found, "at line ", error.line, "offset", error.offset, "\n"].join(" "));
        process.exit(1);
      }
      return {
        id: folder.replace(path.sep, "/"),
        name: name,
        extname: extname,
        folder: folder,
        wd: wd,
        original: original,
        ast: ast
      };
    };

    /*
    Generate all the files in a directory
    */


    CLI.prototype.generateDir = function(wd, folder) {
      var files, input, item, _i, _len, _ref;
      files = [];
      if (folder != null) {
        input = path.join(wd, folder);
      } else {
        input = wd;
      }
      _ref = fs.readdirSync(input);
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        item = _ref[_i];
        if (fs.statSync(path.join(input, item)).isDirectory()) {
          if (folder != null) {
            files.push.apply(files, this.generateDir(wd, path.join(folder, item)));
          } else {
            files.push.apply(files, this.generateDir(wd, item));
          }
        } else if (this.pattern.test(item)) {
          files.push(this.generateFile(wd, folder, item));
        }
      }
      return files;
    };

    return CLI;

  })();

}).call(this);
